...
def main():
    parser = argparse.ArgumentParser(description='Find relation between numbers')
    parser.add_argument('-f', '--filename', dest="filename", help='read combinations from FILE')
    parser.add_argument("-s", "--separator", dest="separator", help="separator between numbers")

    parser.add_argument("-u", "--unique", dest="unique", action="store_true", help="exclude repeated numbers")
    ...
    if filename:
        try:
            ...
            print "Error:%s:reading %s:%s" % (sys.argv[0], e.filename, e.strerror)
            sys.exit(1)
    ...
    for arg_pair in binar_combinations:
        for func, func_name, is_commutative in binar_func_list:
            relation_number, has_relation = get_relation(arg_pair, func, sequence)
            if has_relation:
                binar_relations.append(arg_pair + (func_name, relation_number))

            if not is_commutative:
                relation_number, has_relation = get_relation(arg_pair[::-1], func, sequence)
                if has_relation:
                    binar_relations.append(arg_pair[::-1] + (func_name, relation_number))
